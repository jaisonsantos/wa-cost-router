...
5. **Messages** – envia mensagem (resposta `202 Accepted`), aguarda o processamento com **Messages - Await Job Completion**, lista jobs, consulta detalhes usando `job_id` capturado e executa o cenário opcional **Messages - Rate Limit Demo** para validar respostas `429`. A pasta **Multi-Channel Regression** cobre WhatsApp/SMS/e-mail e asserções garantem que `to_number`/`channel_address` retornem mascarados conforme a política de PII.
