...
|----|--------|------------|----------------|------------|--------------|-------|-----|
| T1 | Vincular rate cards a provedores e corrigir seleção de custos | P0 | Backend | 5d | E1 | Alto (migração de dados) | • Migration Alembic adiciona `provider_id` + backfill idempotente cruzando por nome (fallback seguro).<br>• Atualizar `RoutingEngine`, seeds e importador CSV/API para usar `provider_id` (incluindo `docs/postman/sample_rates.csv`).<br>• Revisar docs: [`DATA_MODEL`](../architecture/DATA_MODEL.md), [`API_REFERENCE`](../api/API_REFERENCE.md), [`MIGRATIONS`](../operations/MIGRATIONS.md).<br>• Postman: request "Rates - Import CSV" com variável `provider_id`, testes garantindo custo > 0.<br>• `make dev` + `make ci` verdes com base limpa e existente. |
